...
- [ ] **`amount` prop deprecation on `PhotocardView`** — still passed from both `reaction.tsx` and `stats.tsx` even though the property is deprecated and no longer drives layout. `todayTotal` in `reaction.tsx` is also computed only to feed it. Removable in a single cleanup pass.
