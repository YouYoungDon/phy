...
- [x] ~~**Income record system integration (sub-spec C)**~~ — landed 2026-05-24. See `SOBAGI_CURRENT_STATE.md` for handoff and `Recently completed` below for commit list.
...
- [ ] **Floating hearts pacing** — charming on record #1, noise by record #30; reduce or remove after N records. Note: the first-record stimulation peak was partly softened 2026-05-24 by differentiating the surprised+tier1 reaction title (no longer double-✨); hearts remain the loudest first-record element.
- [ ] **Zero-state interactivity hint (onboarding-lite)** — fresh-state emotional QA (2026-05-24) flagged that a first-timer sees Sobagi + jar + TV + mailbox with no affordance that anything is tappable. Discovery-over-tutorial is intentional, but consider one *very soft* cue. Keep restraint-first; do not add a tutorial overlay.
- [ ] **Pebble jar "0개" empty-tap wording** — tapping the (barely-visible) empty jar surfaces a quantified `조약돌 0개`. Minor numeric read at zero-state; consider a softer empty-jar line. `src/pages/index.tsx` jar onPress.
- [ ] **dogfooding-fresh-state doc — checkpoint 1 expectation fix** — the doc lists "absence of mailbox red-dot" at first launch, but `PERSONAL_LETTERS['001']` (`triggerDays: 0`) delivers a welcome letter immediately, so a red-dot IS present (intended, warm). Correct the doc's stated expectation. `docs/dogfooding-fresh-state-2026-05-24.md`.
...
- [x] **Fresh-state emotional QA + first-record title polish** — 2026-05-24. Code-level fresh-state walkthrough (8 checkpoints) confirmed the app still reads as "생활의 흔적" (no finance/game/streak drift); no release blockers. One should-polish fix landed: the `surprised`+tier1 reaction title changed from `처음 들렀네요 ✨` → `오늘 첫 걸음이네요 🌿` so the first-record moment no longer echoes the bubble (`오늘 처음 들렀네요 ✨`) with a double ✨ — reduces repetition + softens the congratulatory peak while keeping warmth. `src/pages/reaction.tsx`. 17 suites / 285 tests green. Remaining QA observations logged to Backlog above.
- [x] **Stress-test hardening sweep (9 fixes)** — landed 2026-05-24. Robustness/overflow/race audit. (A) deleteExpense recomputes recordedDaysCount/streak/totalRecordCount; (B) calendar dayAmount clamped to 1 line vs overflow; (C) ref-based double-save guard; (D) income row divider + chevron; (E) collision-resistant `generateExpenseId`; (F) storageService retry + success boolean, saveExpense awaits writes; (G) AppState foreground listener refreshes visit-date anchor across day boundary; (H) `Expense.localDate` captured at creation + `expenseLocalDate` helper routed through all 27 day-grouping sites (tz-stable, behavior-preserving for non-travelers); (I) reaction title kind carried via `emotionStore.lastKind` vs midnight race. 17 suites / 285 tests green. Commits `0a56098` → `d358a72`. (2026-05-24)
- [x] **Sub-spec C post-QA polish sweep (5 fixes)** — landed 2026-05-24. (1) `computeCalmDayCount` now excludes income from daily totals (was contaminating both `getCalmAtmosphereOpacity` and the stats calm observation branch) +4 regression tests. (2) Edit sheet allows amount=0 on income (matched save flow's `금액 (선택)` affordance). (3) `isFirstRecordToday` filters income so a salary log doesn't consume the 'surprised' welcome slot from later spending. (4) `getReactionTitle` gains a kind-aware branch with income-tone titles distinct from `INCOME_REACTION_POOLS` (`오늘은 든든한 날이에요 🌿` / `들어온 날이네요 🍃` / `들어왔네요 🍃`). (5) Income rows on stats screen now display the user's saved `userEmotion` emoji (was previously invisible). 16 suites / 277 tests green. Commits `0a56098` → `82da7da`. (2026-05-24)
- [x] **Income system integration (sub-spec C)** — `evaluateIncome` 2-rule subroutine (hour ≥ 22 → `'sleepy'`, else → `'happy'`); explicit negative tests prove amount / streak / first-of-day / `'surprised'` are never consulted. `INCOME_REACTION_POOLS` (3 tiers × 3 lines, warmth/relief tone, kind-gated selector). `MonthPresenceRow` income-only days render `●`. `hasNightPattern` filters income out. `selectStatsObservation` gains one quiet income branch (`들어온 일이 종종 있었어요 🍃`) at ≥ 2 income days in 30. Memory file `feedback_sobagi_allowance_giving_scene.md` narrowed. No new emotion token; no storage change; no pebble interaction; no `restService` touches. +23 new tests (273 total). Commits `28edb54` → `908b9b3`. (2026-05-24)
