...
**Previous doc:** docs/project-state-2026-05-15.md
...
### Found Item System (`src/services/foundItemService.ts`)

**Implemented 2026-05-17.** The full pipeline — trigger detection, staged delivery, and UI — is live.

**Four trigger conditions:**
1. **Return after gap** — today has ≥ 1 expense, and the most recent prior expense was ≥ 3 calendar days ago
2. **First of month** — today has ≥ 1 expense, and no other day in the current calendar month has any expenses
3. **Quiet yesterday** — yesterday had ≥ 1 expense with a total below 15,000원
4. **Small everyday purchase** — any recent (today or yesterday) cafe/food expense under 6,000원

Triggers are checked in this order; the first match wins. A random unfound item is chosen from those not yet in `foundItemIds`.

**Staged delivery (guarantees delayed discovery):**
1. Trigger fires → item is staged: `STAGED_ITEM_ID` written, `LAST_ITEM_DATE` = today. No dot yet.
2. On the *next app open on a different day*: `promoteStaged()` runs first → moves `STAGED_ITEM_ID` → `PENDING_NEW_ITEM_ID`, clears `STAGED_ITEM_ID`.
3. Amber dot appears on 🎒 bag icon.
4. User opens bag → `pendingNewItemId` moves into `foundItemIds`, dot clears.

**Async timing note:** `promoteStaged` and the home screen's storage load are both async in the same app mount. The dot may appear in the same session as promotion or one session later — never in the same session as the record that triggered staging. This delayed-discovery quality is intentional and preserved.

**Anti-exploitation design:**
- Grace period: `recordedDaysCount < 3` skips all trigger checks entirely
- Cooldown: `LAST_ITEM_DATE` blocks re-staging for 3 calendar days after each staging event. Cooldown begins at staging, not delivery. Bag-open does not reset it.
- Pending or staged item blocks any new staging
- System silently retires when all 8 items are found (`unfound.length === 0`)
- Max ~2 items per week by design

**Found items and bag accumulation are independent systems.** `minDays`-gated bag items and found items use separate storage keys, separate rendering sections, and separate logic. They cannot interfere.

**Storage keys used:**
- `FOUND_ITEM_IDS` — persisted array; grows as user opens bag with a pending item
- `PENDING_NEW_ITEM_ID` — the dot-visible stage
- `STAGED_ITEM_ID` — the pre-delivery stage (no dot)
- `LAST_ITEM_DATE` — staging date, used for cooldown

**UI (in `index.tsx`, unchanged from 2026-05-15):**
- Amber dot on 🎒 when `pendingNewItemId !== null`
- Opening bag moves pending into found, clears pending from state + storage
- "소박이가 두고 간 것" section renders found items as tappable chips
- Tapping a found item shows its `findLine` description
...
sobagi-last-item-date         → string (YYYY-MM-DD, staging date for found item cooldown)
sobagi-staged-item-id         → string | null (found item awaiting next-day promotion)
sobagi-last-visit-date        → string (defined in STORAGE_KEYS, not yet read/written)
sobagi-observation-save-count → number (defined in STORAGE_KEYS, not yet read/written)
sobagi-mailbox-delivered-ids  → string[] (defined in STORAGE_KEYS, not yet read/written)
sobagi-last-bag-open-days     → string (defined in STORAGE_KEYS, not yet read/written)
```
...
| `foundItemService.ts` | `promoteStaged()` + `checkForFoundItem()`: staged delivery pipeline |
...
Calendar day distance in `foundItemService.ts` also uses noon-anchored math to avoid timezone edge cases.
...
### Found Item System — Current State (as of 2026-05-17)

The full pipeline is live. See §3 for complete implementation details.

**Summary:** four natural-behavior triggers → staged delivery (no same-session dot) → amber dot on next-day open → bag discovery. Grace period + 3-day cooldown prevent exploitation. System retires silently after all 8 items are found.

The 8 findable items (`src/constants/findableItems.ts`):

| ID | Emoji | Name | Find Line |
|----|-------|------|-----------|
| f1 | 🍀 | 네잎 클로버 | 산책하다가 발견했어요. 드리고 싶었어요. |
| f2 | ✨ | 반짝이는 조각 | 빛에 반사되는 게 예뻐서요. |
| f3 | 🌰 | 작은 도토리 | 특별한 이유는 없어요. 그냥 챙겨왔어요. |
| f4 | 🎀 | 리본 조각 | 오늘 어딘가에서 떨어져 있던 거예요. |
| f5 | 🍃 | 찻잎 조각 | 차 한 잔 하면서 생각났어요. |
| f6 | 🌸 | 꽃잎 | 바람에 날려왔어요. 받아뒀어요. |
| f7 | 🪶 | 작은 깃털 | 어디서 떨어진 건지 몰라요. 부드러웠어요. |
| f8 | 🐚 | 조개껍데기 | 여기서 이런 게 나올 줄은 몰랐어요. |
...
| Predictable found-item farming | Exploitable triggers (e.g. every Nth day) destroy the surprise quality |
...
2. **Hide closet icon** — until closet has real content, it creates a dead-end
3. **Add `(선택)` to emotion picker label** — parity with memo section
4. **Softening settlement section numbers** — reduce font size/weight; de-emphasize as financial summary
5. **Emotion message pools** — even 2–3 strings per emotion would significantly reduce repetition
6. **Make trend graph bars tappable** — tap a bar to select that day in the calendar
7. **Android keyboard behavior** — investigate whether save button is accessible on Android
...
### On Found Item Design
The found item system works emotionally because it is structurally honest about time. Staging happens now; delivery happens later. The user cannot manufacture discovery by performing the right action — they can only live with the app, and the app notices. When the trigger is a natural behavior (a small coffee, a quiet afternoon, coming back after a week away), the found item feels like Sobagi was paying attention, not like a mechanic being resolved.

---

*This document reflects the state of Sobagi as of 2026-05-17, after the found item trigger activation. Update it when design decisions change or systems move from one status category to another. Do not update it just because code changed — update it when the system's role or status changes.*
